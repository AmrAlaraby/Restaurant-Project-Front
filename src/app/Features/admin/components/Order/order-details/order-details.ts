...
export class OrderDetails implements OnInit {
  ...
  // =========================
  loadMenu() {
    this.menuService.getAll({ pageIndex: 1, pageSize: 1000 }).subscribe((res) => {
  ...
  get availableMenuItems() {
    const existingIds = this.order?.orderItems?.map((x: any) => x.menuItemId) || [];
  ...
  saveNewItems() {
    ...
    this.ordersService.addItemsToOrder(this.orderId!, newItems).subscribe({
      next: () => {
  ...
  removeItem(orderItemId: number) {
    ...
    this.ordersService.removeItemFromOrder(this.orderId!, orderItemId).subscribe({
      next: () => {
        console.log('✅ Removed');
        this.loadOrder();
        this.error = null;
